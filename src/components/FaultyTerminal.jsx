...
vec3 getColor(vec2 p) {
  ...
  vec3 baseColor =
    vec3(0.9) * middle + sum * 0.1 * vec3(1.0) * bar;
  return baseColor;
}
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    try {
      ...
      container.classList.add("faulty-terminal-fallback");
      return () =>
        container.classList.remove("faulty-terminal-fallback");
    }
    ...
    const update = (timestamp) => {
      ...
      if (!pause) {
        const elapsed =
          (timestamp * 0.001 + timeOffsetRef.current) * timeScale;
      ...
      if (pageLoadAnimation && loadAnimationStartRef.current > 0) {
        const animationElapsed =
          timestamp - loadAnimationStartRef.current;
  ...
  return (
    <div
      ref={containerRef}
      className={`faulty-terminal ${className}`.trim()}
      style={style}
      aria-hidden="true"
      {...rest}
    />
